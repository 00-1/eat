...
**Version 0.65 — living document.** This file is the canonical description of how
...
| Version | Date | Change |
|---------|------|--------|
| 0.65 | 2026-07-01 | **Tranche 2 (worker Batch 2) — fruit member expansion.** The Tranche 2 fruits list is mostly covered by the existing **Whole fruit** card (categoryUniformity: mixed), because cohort data groups them (Muraki 2013, Aune 2017, Wang 2014) rather than analysing them alone. Rather than fabricate per-fruit cards with no independent hard-outcome data, we expanded whole-fruit's `members` list from 9 to 18 entries — recording each fruit's position with an honest tag: **good** (evidence-backed: apples/pears, oranges/citrus, grapes, berries); **likely** (shares the profile; sparse per-fruit data: cherries, kiwi, pomegranate, stone fruit, figs, papaya, guava, tropical mango/pineapple, olives the fruit, dried fruit); **weaker** (higher-GI/less clear: bananas, watermelon); **worse** (Muraki 2013 signal: melon/cantaloupe); **unknown** (usually eaten as sweetened juice: cranberries). Concentrated evidence ≠ member-specific effect (the whole-fruit rule from ADDING-FOODS.md). Dried fruit stays in HOLDING as its own line because it plausibly diverges (concentrated sugar loads). No new own-cards created for these items — the honest disposition per PLAN.md is "member of an existing food." All 79 tests green; signal-tier partition unchanged. |
| 0.64 | 2026-07-01 | **Tranche 1 (worker coverage grind) — 6 new foods.** First worker batch on top of Phase 0's signalTier split. Five graduated to full cards (positive · low/moderate) with real hard-outcome cohort meta anchors and PMID-pinned sources; one graduated as a long-tail neutral (no hard-outcome data). **Oats & oatmeal** → positive · moderate — Hu 2020 BMJ (PMID:32641435), oatmeal HR 0.79 (0.75-0.83) for T2D at ≥2 servings/week in ~195,000 across NHS/NHSII/HPFS; Yu 2022 Nutrients (PMID:35631184) β-glucan LDL-C −0.27 mmol/L in 13 RCTs. **Seed oils (canola, sunflower, soybean, etc.)** → positive · low, **contested** — Marklund 2019 CHARGE (PMID:30971107) biomarker meta of 30 cohorts (68,659 participants, 15,198 CVD events): higher circulating LA → CV mortality HR 0.78 (0.70-0.85), total CVD 0.93 (0.88-0.99). Contested flag captures the "seed oils are poison" contrarian narrative; the outcome cohorts disagree with it. **Brown rice** → positive · moderate — Sun 2010 Arch Intern Med (PMID:20548009) in NHS/NHSII/HPFS (~197,228): HR 0.89 (0.81-0.97) for T2D at ≥2 servings/week; 50 g/day brown→white substitution ↓16% T2D; corroborated by Hu 2020. **Peanuts (& peanut butter)** → positive · moderate — Luu 2015 JAMA Intern Med (PMID:25730101) in 3 cohorts (~206k, 14,440 deaths): total mortality HR 0.79 (0.73-0.86) US, 0.83 (0.77-0.88) Shanghai; IHD 0.60-0.70. Cross-linked to Legumes (parent) and echoes the tree-nut mortality signal. **Mushrooms** → positive · low — Ba 2021 Nutr J (PMID:34548082), 5-cohort meta (~601,893): all-cause mortality RR 0.94 (0.91-0.98). Small but graded effect; graduates from HOLDING. **Pasta** → neutral · low (long-tail) — no pasta-specific hard-outcome pooled RR exists; Chiavaroli 2021 BMJ (PMID:34348965) low-GI RCT-meta shows no harm signal for pasta-inclusive low-GI patterns. Recorded honestly as verified:false. **Held in holding**: garlic/other alliums (Zhu 2014 CRC-null; garlic supplements CRC 1.18 signal); onions/honey/spices/seeds/plant-milks/dried fruit; **energy drinks added** (Gualberto 2024 only acute markers). Cross-links: oats + brown-rice → Whole grains, peanuts → Legumes. New signalTier partition: 35 notable + 8 long-tail (pasta joined the long-tail bucket). All 79 tests green. |
| 0.63 | 2026-07-01 | **Phase 0 — signal tiers + long-tail "eat to taste" section.** Added `Scoring.signalTier(food, assessment)`, a pure derived function that partitions every food into `notable` (worth a full card + eligible for the summary panels) or `long-tail` ("eat to taste — no strong signal either way"). Notable iff ANY of: directional verdict / neutral-but-leaning / directional per-outcome verdict / contested / burden ≥ high / mixed uniformity. Attached to each food as `food.signalTier` after burden mapping. Summary panels (`renderHighlights`) and the card grid now filter to notable-only; long-tail foods render as a compact category-grouped list below the grid ("Eat to taste — no strong signal either way"), one row each, tap to expand a brief note or open the full card (pinned into the grid). Search still reaches into the long tail. Anchors held: `trans-fat` notable (directional), `artificial-sweeteners` notable (contested), `red-meat` notable (directional per-outcome), `poultry` long-tail (neutral, no lean). The 8 long-tail foods with today's data match the spec exactly: eggs, poultry, milk, butter, potatoes, coconut-oil, shellfish, fruit-juice. No verdict changes; no new foods; purely the derived tier + UI split + tests. 79 tests green (three new: `signalTier` returns valid values / partition covers every food / anchors). This is the unlock for scale: hundreds of foods can coexist without burying the ~40 that carry the signal. |
| 0.62 | 2026-07-01 | **Full-text verification pass (snippets → papers).** Re-checked every snippet-sourced figure against the actual paper abstracts/full-text via NCBI E-utilities and Europe PMC (unlike the prior pass, this environment has real outbound internet). Corrections: **red-meat T2D** PMID 39174153 (a same-issue Aune editorial) → **39174161** (the actual Li 2024 IPD federated meta), N 1,970,000 → 1,966,444 exact; **poultry** "Shi 2023 Crit Rev Food Sci Nutr" author name → **Papp 2023** (Schwingshackl group, PMID:34542332) — the DOI is correct, the surname was wrong; **tomatoes** headline RR 0.98 → 0.91 anchored to Luo 2021 (PMID:34017849) cohort-only 10-study pooled figure (the previously-recorded 0.96 was in fact Rowles 2018's raw-tomato subgroup, mis-attributed as "cohort-only"); added Rowles 2018 (PMID:29317772) as the mixed-design corroborator with 260,461 participants; **artificial-sweeteners** study citation moved from PLoS Med → BMJ (Debras 2022 has both — cancer in PLoS Med, CVD in BMJ, PMID:36638072), added the newer 12-cohort Queiroz 2025 meta (PMID:39557594, 1.22M, HR 1.14 all-cause mortality) as the stronger meta-level signal while keeping verified:false for the class heterogeneity. **Confirmed exactly against abstracts (no changes):** Xi 2014 fruit juice (137,663 100%-juice / 4,906 cases / RR 1.03 NS), Mousavi 2025 potatoes/french-fries (205,107 / 22,299 T2D / HR 1.20 fries + 1.01 baked/boiled), de Souza 2015 trans-fat (all-cause 1.34, CHD-mort 1.28, total-CHD 1.21; industrial-vs-ruminant split intact), Matheson 2009 ARIC shellfish (13,355 / 1,382 CHD / HR 0.98), Namazi 2019 shellfish T2D (RR 0.95 across 5 studies), Pollock 2016 leafy-greens (RR 0.842 across 8 cohorts), Zhang 2011 cruciferous (quintile HRs match: 1.00/0.91/0.88/0.85/0.78 in N=134,796 Shanghai cohorts; verified quintile midpoints via PMC full text), Qin 2021 fried-foods (major CV 1.28 / CHD 1.22 / I²=82% intact; noted the recorded 1.08 all-cause mortality is Sun 2019 single-cohort, not Qin's pooled 1.03 NS). **Three verified:false foods audited and confirmed as genuine gaps:** coconut oil (Neelakantan 2020 LDL-surrogate only, no hard-outcome cohort exists), white bread (no white-bread-specific hard-outcome pooled RR; the wholegrain contrast is dark-bread 0.79 in Hu 2020, refined-grains null in Aune 2016, and SUN overweight OR 1.40 in de la Fuente-Arrillaga 2014 PMID:25335643), artificial sweeteners (Queiroz 2025 shows meta-level harm but reverse-causation concerns + class heterogeneity keep the class-average verdict uncertain). Each carries a dated revision entry saying old→new where a number moved. All 76 tests green. |
| 0.61 | 2026-07-01 | **Verdict-credibility audit (all 38 foods red-teamed).** Six adversarial reviewers tried to *refute* every verdict from independent literature; 27 came back sound, 0 wrong-direction. Applied: **trans fat** headline RR 1.42 → 1.34 (the 1.42 was mis-attributed to de Souza 2015, whose real figures are all-cause mortality 1.34 / CHD-mortality 1.28 / total CHD 1.21 — still the biggest-harm champion); **soy** certainty Moderate → Low (benefit is Asian-cohort-driven, ~null for breast cancer in Western populations, so it may not transfer to a Western diet); **four foods source-verified** (verified 31 → 35 of 38) — leafy greens (Pollock 2016 CVD RR 0.84 + Aune 2017 subtype), cruciferous (Zhang 2011 + Aune 2017; cancer leg flagged case-control-weighted), tomatoes (cohort intake RR 0.96/0.99 confirming the biomarker-driven-not-intake read), poultry (Shi 2023 24-cohort CVD-null). **Held after review:** avocado stays positive · Low (scored on conservative inputs the engine still computes Low; hand-flipping to very-low/neutral would game the deterministic design — the reviewer's gut call doesn't override computed evidence quality); UPF stays Moderate and olive oil stays Low (reaching the reviewer's suggested tiers would require inventing un-sourced inputs); alcohol stays Low (its tier reflects honestly high heterogeneity/confounding — direction-certainty ≠ evidence-grade). Red meat needed no change — its T2D per-outcome already records Li 2024's *significant* HR 1.10. |
| 0.60 | 2026-07-01 | **Shellfish source-verified (31/38).** Second grounding pass found the CHD anchor: Matheson 2009 (ARIC, PMID:19631050) — 13,355 adults, 1,382 coronary events, high-vs-low shellfish HR 0.98 (0.82–1.18), NS — which pins both the RR *and* the participant count from one source, so `verified` flips to true. T2D corroborated by Namazi 2019 (PMID:31508336, RR 0.95, 0.83–1.10, NS). Headline moved from the T2D meta (0.95) to the ARIC CHD estimate (0.98); at 0.98 the faint favourable lean falls below the directionality floor, so shellfish now reads as **genuinely neutral** rather than "leans slightly beneficial" — resolving the earlier open question about whether that lean was the honest read. |
| 0.59 | 2026-07-01 | **Shellfish T2D figure source-pinned.** Ran the confirming search on the shellfish diabetes figure: Namazi 2019 (*Health Promot Perspect*, PMID:31508336, 7 cohorts) — "seafood other than fish" (incl. shellfish) → T2D RR 0.95 (0.83–1.10), NS. Corrected a potential trap: the "higher risk in women" line is a subgroup, not the pooled estimate, so the neutral verdict stands. Recorded the figure in `sources.pooledRR` but kept `verified:false` — the participant count and the CHD figure are still unconfirmed, so full provenance isn't met yet. |
| 0.58 | 2026-07-01 | **Member ↔ group cross-links.** Where a group card and one of its members both exist (Legumes⊃Soy foods, Fried foods⊃French fries, Whole fruit⊃Berries, Whole grains⊃Wholemeal bread, Refined grains⊃White bread & White rice), the expanded card now shows a navigable cross-link — "◎ Whole food group. These members have their own card: …" on the group, and "Part of a group verdict. This sits inside our verdict for … " on the member — each a jump link (reuses the summary-row jump). Turns the item-vs-group overlap from a confusion into navigation. Bidirectional links derived from a single `CARD_MEMBERS` map (after the name-lift, so link labels are the final displayed names). |
| 0.57 | 2026-07-01 | **Item-vs-group scope tag.** Several cards are whole-class verdicts (legumes, whole fruit, whole grains, fried foods, berries, red/processed meat, poultry, shellfish, soy foods, sugary drinks, refined/ultra-processed, sweeteners, fatty fish, tree nuts, leafy greens, cruciferous) that sit in the same list as their own members (e.g. "Soy foods" under "Legumes", "French fries" under "Fried foods"), which read as peers. Added an explicit per-food `scope` field (`group` vs `item`) and a subtle dashed "◎ food group" chip on group cards and summary rows, with the tooltip "the verdict is the average across its members." Distinct from the `groups.js` super-groups a food *belongs to* — this labels what the card itself *is*. |
| 0.56 | 2026-07-01 | **Three new food items (38 foods).** Grounded and graduated three foods off the holding list. **Fried foods** → negative · low (cardiovascular disease RR 1.28 highest-vs-lowest, Qin 2021 Heart, 17 cohorts ~562k; all-cause mortality HR 1.08, Sun 2019 BMJ) — a method-level verdict sitting alongside the food-specific French-fries item; large relative effect, so it lands on the "worth cutting down" summary. **Fruit juice** → neutral · low (100% juice not associated with type-2 diabetes, RR 1.03 NS, Xi 2014 — the diabetes harm belongs to sugar-sweetened fruit *drinks* at 1.28, not 100% juice; still inferior to whole fruit, Muraki 2013). **Shellfish** → neutral · low with a faint favourable lean (the dietary-cholesterol worry didn't translate to hard outcomes: T2D RR ~0.95 NS, Namazi 2019; CHD ~0.98 NS, ARIC; verified:false pending fuller sourcing). Each carries full treatment (evidence+sources, mechanism, exceptions; dose curves for fried foods & juice). Removed shellfish & fruit juice from the holding list. |
